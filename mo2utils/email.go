...
func VerifyEmailMessage(url string, name string, receivers []string) *emailservice.Mo2Email {
	...
	return &emailservice.Mo2Email{Content: body, Subject: "确认Mo2邮箱!", Receivers: receivers}
}

// SendEmail can send raw email to receivers
func SendEmail(mail *emailservice.Mo2Email, senderAddr string) (err *mo2errors.Mo2Errors) {

	err = emailservice.QueueEmail(mail, senderAddr)
	if err != nil {
		fmt.Println(err)
	}
	return
